...
class UangkuBalanceWidgetV2Provider : HomeWidgetProvider() {
  override fun onReceive(context: Context, intent: Intent) {
    super.onReceive(context, intent)
    if (intent.action == "ACTION_TOGGLE_VISIBILITY_V2") {
      ...
      val componentName = ComponentName(context, UangkuBalanceWidgetV2Provider::class.java)
  ...
  override fun onUpdate(
    ...
    appWidgetIds.forEach { widgetId ->
      val views = RemoteViews(context.packageName, R.layout.uangku_balance_widget_v2)
      ...
      val expenseLabelColor = if (isDarkMode) Color.parseColor("#FFFF9D8E") else Color.parseColor("#FF2D2D2D")
      ...
      views.setTextColor(R.id.widget_expense_label, expenseLabelColor)
      ...
      views.setTextColor(
        ...
        if (isHidden) {
           if (isDarkMode) Color.WHITE else valueColor
        } else {
           if (isDarkMode) Color.WHITE else expenseColor
        },
      )
      ...
      val toggleIntent = Intent(context, UangkuBalanceWidgetV2Provider::class.java).apply {
        action = "ACTION_TOGGLE_VISIBILITY_V2"
      }
